...
public class Controller implements Initializable{
            ...
            Thread thread = new Thread(() -> {
                    try {
                        ...
                        while (true) {
                            String srt = in.readUTF();
                            if (srt.startsWith("/")) {
                                if (srt.equals("/serverclosed")) break;
                                if (srt.startsWith("/clientslist ")) {
    ...
    //метод для успешного закрытия клиента по крестику
    public void Dispose() {
    ...
    public void selectClient(MouseEvent mouseEvent) {
        if(mouseEvent.getClickCount() == 2) {
            Chat.Client.MiniStage ms = new Chat.Client.MiniStage(clientsList.getSelectionModel().getSelectedItem(), out, textAreas);
            ms.show();
        }
    }
}
